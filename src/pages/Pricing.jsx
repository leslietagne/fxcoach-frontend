...
export default function Pricing() {
  ...
  const premiumFeatures = lang === 'EN'
    ? ["✅ Full stats + interactive charts", "✅ All biases detected (personalized AI)", "✅ Full coach report in natural language", "✅ Chat with AI coach", "✅ Progress tracking + FTMO countdown", "✅ Key points for your next trades", "✅ Personal notes saved", "✅ Multilingual (FR/EN)"]
    : ["✅ Stats complètes + graphiques interactifs", "✅ Tous les biais détectés (IA personnalisée)", "✅ Rapport coach complet en langage naturel", "✅ Chat avec le coach IA", "✅ Suivi progression + countdown FTMO", "✅ Key points pour tes prochains trades", "✅ Notes personnelles sauvegardées", "✅ Multilingue (FR/EN)"];
      ...
      <div className="max-w-5xl mx-auto pt-28 pb-16 px-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          ...
          <div className="inline-flex items-center gap-1 bg-gray-100 rounded-xl p-1">
            <button onClick={() => setBilling('monthly')} className={`px-5 py-2 rounded-lg text-sm font-semibold transition ${billing === 'monthly' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-400'}`}>
              {t.monthly}
            </button>
            <button onClick={() => setBilling('yearly')} className={`px-5 py-2 rounded-lg text-sm font-semibold transition flex items-center gap-2 ${billing === 'yearly' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-400'}`}>
              {t.yearly}
              <span className="text-xs px-2 py-0.5 rounded-full text-white font-bold" style={{backgroundColor: '#c9a84c'}}>{t.save}</span>
            </button>
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-2 gap-8 mb-16">
          ...
          {/* FREE */}
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="p-8 rounded-2xl border border-gray-200">
            ...
            <button onClick={() => navigate('/analyse')} className="w-full py-3 rounded-xl border-2 border-gray-200 text-gray-700 font-semibold hover:bg-gray-50 transition">
              {t.startFree}
            </button>
          </motion.div>

          {/* PREMIUM */}
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="p-8 rounded-2xl border-2 relative shadow-lg" style={{borderColor: '#c9a84c'}}>
            ...
            <div className="flex items-baseline gap-2 mb-2">
              <div className="text-5xl font-bold text-gray-900">€{billing === 'monthly' ? monthlyPrice : Math.round(yearlyPrice / 12)}</div>
              <div className="text-gray-400 text-sm">/{lang === 'EN' ? 'month' : 'mois'}</div>
            </div>
            <div className="text-sm text-gray-400 mb-8">
              {billing === 'yearly' ? `€${yearlyPrice} ${lang === 'EN' ? 'billed yearly' : 'facturé annuellement'}` : lang === 'EN' ? 'Billed monthly' : 'Facturé mensuellement'}
            </div>
            <div className="space-y-3 mb-8">
            ...
            <button onClick={handleCheckout} className="w-full py-3 rounded-xl text-white font-semibold transition hover:opacity-90" style={{backgroundColor: '#c9a84c'}}>
              {t.startPremium}
            </button>
          </motion.div>
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          ...
          <div className="space-y-4 max-w-2xl mx-auto">
            {faqs.map((faq, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} className="p-6 rounded-xl border border-gray-100 hover:border-yellow-200 transition">
        ...
        {/* CTA FINAL */}
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mt-16 p-8 rounded-2xl" style={{backgroundColor: '#c9a84c10'}}>
          ...
          <button onClick={() => navigate('/analyse')} className="px-8 py-4 rounded-xl text-white font-semibold text-lg shadow-lg hover:opacity-90 transition" style={{backgroundColor: '#c9a84c'}}>
